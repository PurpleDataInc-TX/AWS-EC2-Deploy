...
def _fix_dir_permissions():
    """Set directory to root:SERVICE_USER 750 so cloudpiadmin can traverse it."""
    import pwd
    try:
        entry = pwd.getpwnam(SERVICE_USER)
        os.chown(SECRETS_DIR, 0, entry.pw_gid)
        os.chmod(SECRETS_DIR, 0o750)
    except KeyError:
        pass  # SERVICE_USER doesn't exist on this machine (dev/local run)
